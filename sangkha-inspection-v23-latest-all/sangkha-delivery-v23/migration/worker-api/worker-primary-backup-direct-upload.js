...
const BUILD_VERSION = "2026-08-27-admin-supervisor-live-v1";
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/supervisor/data") return supervisorData(request, env, origin);
      if (request.method === "GET" && url.pathname === "/admin/dashboard") return adminDashboard(request, env, origin);
      if (request.method === "GET" && url.pathname === "/admin/data") return adminData(request, env, origin);
      if (request.method === "GET" && url.pathname === "/management/data") return managementData(request, env, origin);
      if (request.method === "POST" && url.pathname === "/management/action") return managementAction(request, env, origin);
...
async function adminData(request, env, origin) {
  const user = await requireUser(request, env);
  if (!user) return response({ success: false, message: "Session หมดอายุหรือไม่ถูกต้อง" }, 401, origin);
  if (user.Role !== "ADMIN") return response({ success: false, message: "ไม่มีสิทธิ์ดูข้อมูล Admin" }, 403, origin);
  const [users, locations, logs, settings] = await Promise.all([
    readSheet(env, USERS_SHEET),
    readSheet(env, LOCATIONS_SHEET),
    readSheet(env, LOGS_SHEET),
    readSheet(env, SETTINGS_SHEET),
  ]);
  const today = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Bangkok" }).format(new Date());
  const completedLogs = logs.filter((log) => normalizeDate(log.Date, env) === today);
  const completedIds = new Set(completedLogs.map((log) => normalize(log.Location_ID)).filter(Boolean));
  ...
  return response({
    ...
    pending: Math.max(0, locations.length - completedIds.size),
    totalLocations: locations.length,
    average: scores.length ? Number((scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(1)) : 0,
    user: publicUser(user),
    users: users.map(publicDataRow),
    locations: locations.map(publicDataRow),
    logs: logs.map(publicDataRow),
    settings: settings.map(publicSettingsRow),
    scoreSummary: buildAdminScoreSummary(logs, locations, env),
  }, 200, origin);
}

async function supervisorData(request, env, origin) {
  const user = await requireUser(request, env);
  if (!user) return response({ success: false, message: "Session หมดอายุหรือไม่ถูกต้อง" }, 401, origin);
  if (user.Role !== "SUPERVISOR") return response({ success: false, message: "ไม่มีสิทธิ์ดูข้อมูล Supervisor" }, 403, origin);
  const [users, locations, logs] = await Promise.all([
    readSheet(env, USERS_SHEET),
    readSheet(env, LOCATIONS_SHEET),
    readSheet(env, LOGS_SHEET),
  ]);
  const accessible = locations.filter((location) => canAccessSupervisor(user, location));
  const allowedIds = new Set(accessible.map((location) => normalize(location.Location_ID)));
  const scopedLogs = logs.filter((log) => allowedIds.has(normalize(log.Location_ID)));
  const grade = normalizeGrade(user.Assigned_Grade);
  const committee = users.filter((item) => normalize(item.Role) === "INSPECTOR" && (grade === "ALL" || normalizeGrade(item.Assigned_Grade) === grade));
  return response({
    success: true,
    today: new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Bangkok" }).format(new Date()),
    user: publicUser(user),
    locations: accessible.map(publicDataRow),
    logs: scopedLogs.map(publicDataRow),
    committee: committee.map(publicDataRow),
    scoreSummary: buildAdminScoreSummary(scopedLogs, accessible, env),
  }, 200, origin);
}

function publicSettingsRow(row) {
  return Object.fromEntries(Object.entries(row).filter(([key]) => /active_window|test_mode/i.test(key)));
}

function publicDataRow(row) {
  return Object.fromEntries(Object.entries(row).filter(([key]) => !/(pin|password|secret|token|base64|photodata)/i.test(key)));
}
...
function round1(value) { return Number.isFinite(Number(value)) ? Number(Number(value).toFixed(1)) : 0; }
function buildAdminScoreSummary(logs, locations, env) {
  const locationById = new Map((locations || []).map((row) => [normalize(row.Location_ID), row]));
  const today = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Bangkok" }).format(new Date());
  const todayDate = new Date(`${today}T00:00:00Z`);
  const mondayOffset = (todayDate.getUTCDay() + 6) % 7;
  const weekDate = new Date(todayDate); weekDate.setUTCDate(weekDate.getUTCDate() - mondayOffset);
  const weekStart = weekDate.toISOString().slice(0, 10);
  const monthStart = `${today.slice(0, 7)}-01`;
  const valid = (logs || []).map((log) => {
    const date = normalizeDate(log.Date, env);
    const location = locationById.get(normalize(log.Location_ID)) || {};
    const score = scoreAsPercent(log);
    if (!date || !Number.isFinite(score)) return null;
    return { log, date, score, location, locationId: normalize(log.Location_ID) || "ไม่ระบุ", inspectorId: normalize(log.Inspector_ID || log.Student_ID) || "ไม่ระบุ", type: normalizeType(log.Type || log.Assigned_Type || location.Type), grade: normalizeGrade(log.Grade_Level || location.Grade_Level) };
  }).filter(Boolean);
  function periodRows(predicate) {
    const selected = valid.filter(predicate);
    const byLocation = new Map();
    const byInspectorLocation = new Map();
    selected.forEach((item) => {
      const locationKey = `${item.type}|${item.locationId}`;
      const locationBucket = byLocation.get(locationKey) || { type: item.type, locationId: item.locationId, locationName: item.location.Location_Name || item.locationId, grade: item.grade, scores: [], inspectors: new Map() };
      locationBucket.scores.push(item.score);
      const inspectorBucket = locationBucket.inspectors.get(item.inspectorId) || { inspectorId: item.inspectorId, scores: [] };
      inspectorBucket.scores.push(item.score); locationBucket.inspectors.set(item.inspectorId, inspectorBucket); byLocation.set(locationKey, locationBucket);
      const personKey = `${item.inspectorId}|${locationKey}`;
      const person = byInspectorLocation.get(personKey) || { inspectorId: item.inspectorId, type: item.type, locationId: item.locationId, scores: [] };
      person.scores.push(item.score); byInspectorLocation.set(personKey, person);
    });
    const average = (values) => values.length ? round1(values.reduce((a, b) => a + b, 0) / values.length) : 0;
    return {
      submitted: selected.length,
      average: average(selected.map((item) => item.score)),
      rooms: [...byLocation.values()].map((bucket) => ({ type: bucket.type, locationId: bucket.locationId, locationName: bucket.locationName, grade: bucket.grade, count: bucket.scores.length, average: average(bucket.scores), inspectors: [...bucket.inspectors.values()].map((person) => ({ inspectorId: person.inspectorId, count: person.scores.length, average: average(person.scores) })) })).sort((a, b) => `${a.type}|${a.locationId}`.localeCompare(`${b.type}|${b.locationId}`)),
      inspectorLocations: [...byInspectorLocation.values()].map((person) => ({ inspectorId: person.inspectorId, type: person.type, locationId: person.locationId, count: person.scores.length, average: average(person.scores) })).sort((a, b) => `${a.type}|${a.locationId}|${a.inspectorId}`.localeCompare(`${b.type}|${b.locationId}|${b.inspectorId}`)),
    };
  }
  return { today, weekStart, monthStart, periods: { daily: periodRows((item) => item.date === today), weekly: periodRows((item) => item.date >= weekStart && item.date <= today), monthly: periodRows((item) => item.date >= monthStart && item.date <= today) } };
}
...
async function managementData(request, env, origin) {
  const user = await requireUser(request, env);
  if (!user || !["ADMIN", "SUPERVISOR"].includes(user.Role)) return response({ success: false, message: "ไม่มีสิทธิ์ดูข้อมูลจัดการ" }, user ? 403 : 401, origin);
  const [users, locations, logs] = await Promise.all([readSheet(env, USERS_SHEET), readSheet(env, LOCATIONS_SHEET), readSheet(env, LOGS_SHEET)]);
  const accessible = user.Role === "ADMIN" ? locations : locations.filter((row) => canAccessSupervisor(user, row));
  const allowedIds = new Set(accessible.map((row) => normalize(row.Location_ID)));
  const scopedLogs = user.Role === "ADMIN" ? logs : logs.filter((row) => allowedIds.has(normalize(row.Location_ID)));
  return response({ success: true, role: user.Role, user: publicUser(user), users: user.Role === "ADMIN" ? users.map(publicDataRow) : users.filter((row) => normalize(row.Role) === "INSPECTOR" && (normalizeGrade(user.Assigned_Grade) === "ALL" || normalizeGrade(row.Assigned_Grade) === normalizeGrade(user.Assigned_Grade))).map(publicDataRow), locations: accessible.map(publicDataRow), logs: scopedLogs.map(publicDataRow) }, 200, origin);
}

async function managementAction(request, env, origin) {
  const user = await requireUser(request, env);
  if (!user || !["ADMIN", "SUPERVISOR"].includes(user.Role)) return response({ success: false, message: "ไม่มีสิทธิ์ดำเนินการ" }, user ? 403 : 401, origin);
  const body = await request.json();
  const action = String(body?.action || "").trim().toLowerCase();
  const adminOnly = ["user_create", "user_disable", "location_create", "location_disable", "settings_update", "approval_update", "calendar_event", "override", "export_audit"];
  if (adminOnly.includes(action) && user.Role !== "ADMIN") return response({ success: false, message: "การดำเนินการนี้สงวนสำหรับ Admin" }, 403, origin);
  let result;
  if (action === "user_create") result = await appendObjectRow(env, USERS_SHEET, body.record || {});
  else if (action === "user_disable") result = await updateRowField(env, USERS_SHEET, "Student_ID", body.studentId, "Inspector_Status", "INACTIVE");
  else if (action === "location_create") result = await appendObjectRow(env, LOCATIONS_SHEET, body.record || {});
  else if (action === "location_disable") result = await updateRowField(env, LOCATIONS_SHEET, "Location_ID", body.locationId, "Status", "INACTIVE");
  else if (action === "settings_update") result = await appendObjectRow(env, SETTINGS_SHEET, body.record || {});
  else if (action === "approval_update") result = await appendNamedObjectRow(env, "Approval_Requests", { ...body.record, Updated_By: user.Student_ID, Updated_At: new Date().toISOString() });
  else if (action === "calendar_event") result = await appendNamedObjectRow(env, "Calendar_Events", { ...body.record, Created_By: user.Student_ID, Created_At: new Date().toISOString() });
  else if (action === "attendance_update") result = await appendNamedObjectRow(env, "Attendance_Log", { ...body.record, Recorded_By: user.Student_ID, Recorded_At: new Date().toISOString() });
  else if (action === "substitute_request") result = await appendNamedObjectRow(env, "Substitute_Requests", { ...body.record, Requested_By: user.Student_ID, Requested_At: new Date().toISOString() });
  else if (action === "score_review") result = await appendNamedObjectRow(env, "Score_Requests", { ...body.record, Requested_By: user.Student_ID, Requested_At: new Date().toISOString() });
  else if (action === "repair_update") result = await appendNamedObjectRow(env, "Repair_Requests", { ...body.record, Reported_By: user.Student_ID, Reported_At: new Date().toISOString() });
  else if (action === "override") result = await appendNamedObjectRow(env, "System_Overrides", { ...body.record, Issued_By: user.Student_ID, Issued_At: new Date().toISOString() });
  else if (action === "export_audit") result = await appendNamedObjectRow(env, "Audit_Log", { Action: "EXPORT", Export_Date: body.date || "", Actor_ID: user.Student_ID, Created_At: new Date().toISOString() });
  else return response({ success: false, message: "ไม่รู้จักการดำเนินการนี้" }, 400, origin);
  return response({ success: true, action, result: result || null, message: "บันทึกการดำเนินการแล้ว" }, 200, origin);
}

async function sheetHeaders(env, sheetName) {
  const token = await googleToken(env);
  const range = encodeURIComponent(`${sheetName}!1:1`);
  const result = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${env.SPREADSHEET_ID}/values/${range}`, { headers: { Authorization: `Bearer ${token}` } });
  if (!result.ok) return [];
  const rows = (await result.json()).values || [];
  return (rows[0] || []).map((value) => String(value || "").trim());
}

async function appendObjectRow(env, sheetName, record) {
  const headers = await sheetHeaders(env, sheetName);
  if (!headers.length) throw new Error(`ไม่พบหัวตาราง ${sheetName}`);
  const row = headers.map((header) => record[header] ?? "");
  return appendRow(env, sheetName, row);
}

async function updateRowField(env, sheetName, keyHeader, keyValue, fieldHeader, fieldValue) {
  const token = await googleToken(env);
  const range = encodeURIComponent(`${sheetName}!A:Z`);
  const result = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${env.SPREADSHEET_ID}/values/${range}`, { headers: { Authorization: `Bearer ${token}` } });
  if (!result.ok) throw new Error(`Sheets read failed: ${result.status}`);
  const values = (await result.json()).values || [];
  const headers = values[0] || [];
  const keyIndex = headers.indexOf(keyHeader);
  const fieldIndex = headers.indexOf(fieldHeader);
  if (keyIndex < 0 || fieldIndex < 0) throw new Error(`ไม่พบคอลัมน์ ${keyHeader} หรือ ${fieldHeader}`);
  const rowIndex = values.slice(1).findIndex((row) => String(row[keyIndex] || "").trim().toLowerCase() === String(keyValue || "").trim().toLowerCase());
  if (rowIndex < 0) throw new Error(`ไม่พบรายการ ${keyValue}`);
  const rowNumber = rowIndex + 2;
  const updated = [...(values[rowNumber - 1] || [])];
  while (updated.length < headers.length) updated.push("");
  updated[fieldIndex] = fieldValue;
  const target = encodeURIComponent(`${sheetName}!A${rowNumber}:${String.fromCharCode(65 + Math.max(headers.length - 1, 0))}${rowNumber}`);
  const write = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${env.SPREADSHEET_ID}/values/${target}?valueInputOption=USER_ENTERED`, { method: "PUT", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ values: [updated] }) });
  if (!write.ok) throw new Error(`Sheets update failed: ${write.status}`);
  return write.json();
}

async function ensureSheet(env, sheetName, headers) {
  const token = await googleToken(env);
  const meta = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${env.SPREADSHEET_ID}?fields=sheets.properties.title`, { headers: { Authorization: `Bearer ${token}` } });
  if (!meta.ok) throw new Error(`Sheets metadata failed: ${meta.status}`);
  const titles = ((await meta.json()).sheets || []).map((sheet) => sheet.properties?.title);
  if (!titles.includes(sheetName)) {
    const create = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${env.SPREADSHEET_ID}:batchUpdate`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ requests: [{ addSheet: { properties: { title: sheetName } } }] }) });
    if (!create.ok) throw new Error(`Sheets create sheet failed: ${create.status}`);
    const target = encodeURIComponent(`${sheetName}!A1:${String.fromCharCode(64 + headers.length)}1`);
    await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${env.SPREADSHEET_ID}/values/${target}?valueInputOption=RAW`, { method: "PUT", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ values: [headers] }) });
  }
}

async function appendNamedObjectRow(env, sheetName, record) {
  const headers = Object.keys(record).filter((key) => !/(pin|password|secret|token|base64|photodata)/i.test(key));
  await ensureSheet(env, sheetName, headers.length ? headers : ["Created_At"]);
  const actualHeaders = await sheetHeaders(env, sheetName);
  const row = actualHeaders.map((header) => record[header] ?? "");
  return appendRow(env, sheetName, row);
}
